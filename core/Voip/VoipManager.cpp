...
#else
        return NULL;
    #warning bitmapHandleFromRGBData is undefined
#endif//WIN32
    }
...
namespace {
    inline voip_manager::CallKey getHash(const std::string& account_id, const std::string& user_id) {
        VOIP_ASSERT_RETURN_VAL(!account_id.empty(), 0);
        VOIP_ASSERT_RETURN_VAL(!user_id.empty(), 0);

        std::string name = account_id + "#" + user_id;
        VOIP_ASSERT_RETURN_VAL(!name.empty(), 0);
    ...
    std::string getProtoName(const std::string& account_uid) {
        VOIP_ASSERT_RETURN_VAL(!account_uid.empty(), "");
...
#else
        return NULL;
    #warning need getFilePath realization
#endif//WIN32
    }

    std::string getPhoneNumberFromUid(const std::string& uid) {
        const size_t pos = (size_t)uid.find(PSTN_POSTFIX);
...
namespace voip_manager {
    ...
    class VoipManagerImpl 
        ...
        struct VoipSettings {
            bool enableRtpDump;
            VoipSettings() : enableRtpDump(false) { }
        };
    ...
    const VoipManagerImpl::VoipSettings VoipManagerImpl::_getVoipSettings() const {
        ...
        VOIP_ASSERT_RETURN_VAL(fileStream.is_open(), settings);
...
#ifdef _WIN32
            VOIP_ASSERT_RETURN_VAL(!stat_file_path.empty(), false);
#endif

#ifdef _WIN32
            app_name = "icq.desktop " + core::tools::version_info().get_version();
#else
//    #error need _init_stat realization
#endif//WIN32
    ...
    bool VoipManagerImpl::get_device_list(voip2::DeviceType device_type, std::vector<device_description>& dev_list) {
        ...
        for (unsigned ix = 0; ix < dev_count; ix++) {
            VOIP_ASSERT_ACTION(engine->GetDevice(device_type, ix, _device_name, _device_guid), continue);
    ...
    void VoipManagerImpl::_protocolSendAlloc(const char* data, unsigned size) {
        ...
        _dispatcher.excute_core_context([dataBuf, &dispatcher, this] () {
            ...
            auto onErrorHandler = [__imPtr, packet] (int32_t err) {
            ...
            resultHandler->on_result_ = [packet, __imPtr] (int32_t _error) {
                auto ptrIm = __imPtr.lock();
                VOIP_ASSERT_RETURN(!!ptrIm);
                ...
                if (!!response && response->available()) {
                    ...
                    ptrIm->on_voip_proto_msg(true, (const char*)response->read(size), size);
                }
            };
        });
    }
    ...
    void VoipManagerImpl::_protocolSendMessage(const VoipProtoMsg& data) {
        ...
        _dispatcher.excute_core_context([data, &dispatcher, this] () {
            ...
            resultHandler->on_result_ = [packet, __imPtr, data] (int32_t _error) {
                auto ptrIm = __imPtr.lock();
                VOIP_ASSERT_RETURN(!!ptrIm);
    ...
    void VoipManagerImpl::SessionEvent(std::string account_uid, std::string user_id, voip2::SessionEvent sessionEvent) {
        ...
        auto notification_sender = [this, &contact_ex, &user_id] (eNotificationTypes nt, bool ip) {
            if (kNotificationType_Undefined != nt) {
                if (ip) {
#ifdef _WIN32
                    SIGNAL_NOTIFICATOION(nt, &getPhoneNumberFromUid(user_id));
#endif
                } else {
