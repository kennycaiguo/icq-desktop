...
{    
        ...
        uin_("")
    {
        ...
        */
        ...
        connect(Ui_->show_ignore_list_label, &TextEmojiWidget::clicked, [this]()
        {
            ...
            auto x = Ui_->avatar->mapToGlobal(Ui_->avatar->pos()).x() - ::ContactList::dip(19).px();
            auto y = Ui_->avatar->mapToGlobal(Ui_->avatar->pos()).y() + ::ContactList::dip(15).px();
    ...
    void ProfileSettingsWidget::updateInterface(const QString &uin)
    {
        ...
        Logic::GetContactListModel()->get_contact_profile(uin, [this](Logic::profile_ptr _profile)
        {
            parse(_profile);
        });
    }
