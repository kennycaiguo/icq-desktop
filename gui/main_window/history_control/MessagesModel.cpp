...
namespace Logic
{
    ...
    void MessagesModel::createImagePreviewWidget(Ui::MessageItem &messageItem, QWidget* parent, const Data::MessageBuddy& messageBuddy) const
    {
        ...
        auto item = new HistoryControl::ImagePreviewWidget(parent, messageBuddy.IsOutgoing(), uri.toString(), fullText, previewsEnabled);
	...
	{
		auto item = new HistoryControl::StickerWidget(parent, messageBuddy.GetSticker(), messageBuddy.IsOutgoing());
	...
	QString MessagesModel::formatRecentsText(const Data::MessageBuddy &buddy) const
	{
        ...
        if (buddy.IsFileSharing())
        {
            ...
            auto item = new HistoryControl::FileSharingWidget(buddy.GetFileSharing());
            messageItem->setContentWidget(item);
        }
        else if (buddy.ContainsPreviewableLink())
        {
        ...
        {
            auto item = new HistoryControl::StickerWidget();
            messageItem->setContentWidget(item);
        }
        else
        {
