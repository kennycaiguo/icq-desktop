...
namespace Ui
{
    ...
    void MessagesScrollArea::insertWidget(const int32_t pos, QWidget *widget)
    {
        assert(pos >= 0);
        assert(widget);
        assert(widget->parent() == this);

        Layout_->insertWidget(pos, widget);
        ...
        else if (e->type() == QEvent::TouchUpdate)
        {
            ...
            if (te->touchPointStates() & Qt::TouchPointMoved)
            {
                ...
                if (!newPoint.isNull())
                {
                    ...
                    const auto wheelMultiplier = -15.5;
    ...
    {
        assert(oldSize.height() != newSize.height());
    }
