...
namespace HistoryControl
{
    ...
    FileSharingWidget::FileSharingWidget(const FileSharingInfoSptr& fsInfo)
        : PreviewContentWidget(0, false, QString(), false)
        , FsInfo_(fsInfo)
	...
	FileSharingWidget::FileSharingWidget(
		...
		: PreviewContentWidget(parent, isOutgoing, QString(), previewsEnabled)
	...
	bool FileSharingWidget::canStartImageDownloading() const
	{
		return isState(State::ImageFile_FullPreviewLoaded) ||
	...
	void FileSharingWidget::checkLocalCopyExistence()
	{
		...
		const auto procId = (int)Ui::GetDispatcher()->downloadSharedFile(
			ContactUin_,
			FsInfo_->GetUri(),
            Ui::get_gui_settings()->get_value<QString>(settings_download_directory, ""),
	...
	void FileSharingWidget::mouseMoveEvent(QMouseEvent *event)
	{
		...
		{
			if (isOverControlButton(event->pos()) &&
				!isState(State::PlainFile_Downloaded) &&
				!isState(State::PlainFile_Uploaded) &&
		...
		{
			if (isOverOpenDownloadsDirButton(event->pos()))
			{
				setCursor(Qt::PointingHandCursor);
				return;
			}
	...
	void FileSharingWidget::mouseReleaseEvent(QMouseEvent *event)
	{
		...
		const auto clickedOnButton = (isControlButtonVisible() && isOverControlButton(pos));
		...
		const auto isOpenDownloadsDirButtonClicked = (isOpenDownloadsDirButtonVisible() &&
													  isOverOpenDownloadsDirButton(pos));
		...
		if (isState(State::ImageFile_Downloaded))
		{
	...
	const QRect& FileSharingWidget::getControlButtonPreviewRect() const
	{
		...
		if (Private_.ControlButtonPreviewRect_.isEmpty())
		{
			...
			const QRect widgetRect(QPoint(0, 0), size());
	...
	void FileSharingWidget::setInitialWidgetSizeAndState()
	{
		...
		if(ext.isEmpty() || !isImageExtension(ext))
		{
			setState(State::PlainFile_Initial);
			return;
		}
			...
			case State::ImageFile_Downloading:
				{
					...
					assert(isImageDownloaded || isImageMetainfoLoaded);
	...
	void FileSharingWidget::setFileInfo(qint64 seq, QString rawUri, QString miniPreviewUri, QString fullPreviewUri, QString downloadUri, QString filename, qint64 size, bool played)
	{
	...
	void FileSharingWidget::imageDownloaded(qint64 seq, QString uri, QPixmap preview)
	{
		assert(!uri.isEmpty());
...
//	const QSizeF& getMaxPreviewSize()
//	{
//		static const QSizeF MAX_PREVIEW_SIZE(Utils::scale_value(640), Utils::scale_value(320));
//		return MAX_PREVIEW_SIZE;
//	}
//
//	const QSizeF& getMinPreviewSize()
//	{
//		static const QSizeF MIN_PREVIEW_SIZE(Utils::scale_value(48), Utils::scale_value(48));
