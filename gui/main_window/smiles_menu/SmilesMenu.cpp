...
namespace Ui
{
    ...
    StickersTable::StickersTable(QWidget* _parent, std::shared_ptr<stickers::set> _stickers_set)
        :	QWidget(_parent),
        need_height_(0),
        stickers_set_(_stickers_set),
        column_count_(0),
        row_count_(0),
        max_row_count_(-1)
    {
    ...
    void StickersTable::set_max_row_count(int _val)
    {
        max_row_count_ = _val;
    }
    ...
    bool StickersTable::resize(const QSize& _size, bool _force)
    {
        ...
        int stickers_count = (int)stickers_set_->get_stickers().size();
    ...
    {
        for (uint32_t i = 0; i < stickers_set_->get_stickers().size(); i++)
        {
            if ((stickers_set_->get_stickers().at(i)->get_id() == _sticker_id) &&
                (stickers_set_->get_stickers().at(i)->get_set_id() == _set_id))
            {
    ...
    void StickersTable::paintEvent(QPaintEvent* _e)
    {
        ...
        for (uint32_t i = 0; i < stickers_set_->get_stickers().size(); i++)
        {
            ...
            auto image = stickers_set_->get_sticker_image(i, core::sticker_size::small);
            if (image.isNull())
            {
                continue;
            }
    ...
    void StickersTable::mousePressEvent(QMouseEvent* _e)
    {
        for (uint32_t i = 0; i < stickers_set_->get_stickers().size(); i++)
        {
            ...
            {
                emit sticker_selected(stickers_set_->get_stickers().at(i));
            }
        }
    ...
    void StickersWidget::insert_next_set(stickers::set_sptr _stickers_set)
    {
        int32_t set_id = _stickers_set->get_id();


        auto stickers_view = new StickersTable(this, _stickers_set);
        auto button = toolbar_->addButton(_stickers_set->get_icon());
        button->AttachView(AttachedView(stickers_view, this));

        connect(button, &TabButton::clicked, [this, set_id]()
        {
            emit scroll_to_set(set_tables_[set_id]->geometry().top());
        });

        QLabel* set_header = new QLabel(this);
        set_header->setObjectName("set_header");
        set_header->setText(_stickers_set->get_name());
        v_layout_->addWidget(set_header);

        v_layout_->addWidget(stickers_view);
        set_tables_[_stickers_set->get_id()] = stickers_view;
        Utils::grabTouchWidget(stickers_view);

        connect(stickers_view, &StickersTable::sticker_selected, [this, stickers_view](std::shared_ptr<stickers::sticker> _sticker)
        {
            emit sticker_selected(_sticker);
        });
    }
    ...
    void StickersWidget::init()
    {
        ...
        for (auto stickers_set : stickers::get_stickers_sets())
        {
            insert_next_set(stickers_set);
        }
    ...
    void RecentsWidget::init()
    {
        ...
        stickers_ = std::make_shared<stickers::set>(20);
        stickers_view_ = new StickersTable(this, stickers_);
        ...
        connect(stickers_view_, &StickersTable::sticker_selected, [this](std::shared_ptr<stickers::sticker> _stiker)
        {
            emit sticker_selected(_stiker);
        });
    }

    void RecentsWidget::add_sticker(std::shared_ptr<stickers::sticker> _sticker)
    {
        init();

        if (stickers_->add_sticker(_sticker))
        {
            stickers_view_->on_sticker_added();

            std::vector<int32_t> v_stickers;
            v_stickers.reserve(stickers_->get_stickers().size() * 2);
            for (uint32_t i = 0; i < stickers_->get_stickers().size(); i++)
            {
                v_stickers.push_back(stickers_->get_stickers().at(i)->get_set_id());
                v_stickers.push_back(stickers_->get_stickers().at(i)->get_id());
            }
    ...
    void RecentsWidget::init_stickers_from_settings()
    {
        ...
        for (uint32_t i = 0; i < sticks.size();)
        {
            ...
            auto stick = stickers::get_sticker(set_id, sticker_id);
            if (!stick)
            {
                assert(false);
                continue;
            ...
            stickers_->push_back_sticker(stick);
        }
        ...
        , sticker_meta_requested_(false)
    {
        ...
        connect(Ui::GetDispatcher(), SIGNAL(login_complete()), this, SLOT(login_complete()), Qt::QueuedConnection);
    }
    ...
    void SmilesMenu::login_complete()
    {
        int scale = (int) (Utils::get_scale_coefficient() * 100.0);
        scale = Utils::scale_bitmap(scale);
    ...
    void SmilesMenu::InitResents()
    {
        ...
        connect(recents_view_, &RecentsWidget::sticker_selected, [this](std::shared_ptr<stickers::sticker> _stiker)
        {
            emit sticker_selected(_stiker->get_set_id(), _stiker->get_id());
        });
    }
    ...
    void SmilesMenu::InitStickers()
    {
        ...
        connect(stickers_view_, &StickersWidget::sticker_selected, [this](std::shared_ptr<stickers::sticker> _stiker)
        {
            emit sticker_selected(_stiker->get_set_id(), _stiker->get_id());

            recents_view_->add_sticker(_stiker);
        });
