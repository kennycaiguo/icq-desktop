...
chat_event_data_uptr chat_event_data::make_mchat_event(const rapidjson::Value& _node)
{
    ...
    const auto has_myself_in_members = (members.count(sender_aimid) > 0);
    const auto is_invite = (type == chat_event_type::mchat_invite);
    const auto is_server_issue = (has_myself_in_members && is_invite);
    if (is_server_issue)
    {
        type = chat_event_type::mchat_add_members;
        members.clear();
        members.emplace(sender_aimid);
    }
...
void history_message::serialize(icollection* _collection, const time_t _offset) const
{
	coll_helper coll(_collection, false);
...
namespace
{
	...
	std::string extract_new_file_sharing_file_id(const std::string &uri)
	{
	...
	void serialize_metadata_from_uri(const std::string &uri, Out coll_helper &coll)
	{
		...
		const auto id = extract_new_file_sharing_file_id(uri);
		if (id.empty())
		{
			return;
		}
		...
		const auto is_image = (content_type == '0');
