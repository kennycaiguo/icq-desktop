...
namespace HistoryControl
{
    ...
    StickerWidget::StickerWidget()
        : MessageContentWidget(0, false)
    {
    }

	StickerWidget::StickerWidget(QWidget *parent, const StickerInfoSptr &info, const bool isOutgoing)
    ...
    QPoint StickerWidget::deliveryStatusOffsetHint(const int32_t statusLineWidth) const
    {
        if (Sticker_.isNull())
        {
            return QPoint(
                Utils::scale_bitmap(getStickerMaxHeight()),
                0
            );
        }
	...
	void StickerWidget::render(QPainter &p)
	{
		...
		{
			p.setPen(Qt::black);
			p.fillRect(rect(), Qt::Dense7Pattern);
		}
		else
		{
            updateStickerSize();
