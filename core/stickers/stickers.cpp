...
namespace core
{
    namespace stickers
    {
            ...
            for (auto iter = sets_.cbegin(); iter != sets_.cend(); ++iter)
            {
                ...
                for (auto iter_sticker = map_stickers.cbegin(); iter_sticker != map_stickers.cend(); iter_sticker++)
                {
                    ...
                    for (auto iter_size = sizes.cbegin(); iter_size != sizes.cend(); iter_size++)
                    {
                        ...
                        if (!boost::filesystem::exists(path_for_file))
                            stickers_tasks_.push_back(download_task(ss_url.str(), file_name, set_id, sticker_id, iter_size->second.get_size()));
                    }
                }
            }
        }
        ...
        bool cache::sticker_loaded(const download_task& _task, /*out*/ std::list<int64_t>& _requests)
        {
            for (auto iter = stickers_tasks_.begin(); iter != stickers_tasks_.end(); ++iter)
            {
        ...
        std::shared_ptr<result_handler<const load_result&>> face::load_from_local()
        {
