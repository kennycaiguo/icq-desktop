...
namespace Ui
{
        ...
        , ButtonEventFilter_(new ButtonEventFilter())
	{
        ...
        recents_view_->setUniformItemSizes(true);
...
/*cl? mb recents?*/		connect(contact_list_view_->verticalScrollBar(), SIGNAL(valueChanged(int)), Logic::GetContactListModel(), SLOT(scrolled(int)), Qt::QueuedConnection);
	...
	void ContactList::setSearchMode(bool search)
	{
		...
		else
		{
			...
			stacked_widget_->currentWidget()->setEnabled(false);
			QTimer::singleShot(500, [this]()
			{
				stacked_widget_->currentWidget()->setEnabled(true);
			});
		}
	}
    ...
    void ContactList::dropFiles(const QPoint& pos, const QList<QUrl> files)
    {
        ...
        else if (CurrentTab_  == ALL)
        {
            ...
            {
                index = contact_list_view_->indexAt(pos);
                if (index.row() != 0) //add contact button
                {
	...
	void ContactList::show_recents_popup_menu(const QModelIndex& _current)
	{
		...
		if (dlg.UnreadCount_ != 0)
			popup_menu_->addActionWithIcon(QIcon(Utils::parse_image_name(":/resources/dialog_markread_100.png")), QT_TRANSLATE_NOOP("context_menu", "Mark as read"), makeData("recents/mark_read", aimId));
		...
		popup_menu_->addActionWithIcon(QIcon(Utils::parse_image_name(":/resources/dialog_ignore_100.png")), QT_TRANSLATE_NOOP("context_menu", "Ignore"), makeData("recents/ignore", aimId));
		...
		popup_menu_->addActionWithIcon(QIcon(Utils::parse_image_name(":/resources/dialog_closechat_all_100.png")), QT_TRANSLATE_NOOP("context_menu", "Close all"), makeData("recents/close_all"));
	...
	void ContactList::show_popup_menu(QAction* _action)
	{
		...
		{
			Logic::GetContactListModel()->ignore_contact(aimId, true);
		...
		else if (command == "recents/close_all")
		{
			Logic::GetRecentsModel()->hideAll();
		}
		...
		{
			Logic::GetContactListModel()->block_spam_contact(aimId);
			Logic::GetContactListModel()->remove_contact_from_contact_list(aimId);
		}
    ...
    void ContactList::showNoContactsYet(QWidget *parent, QWidget *list, QLayout *layout)
    {
        ...
        if (!noContactsYet_)
        {
            ...
            noContactsYet_->setStyleSheet("background-color: #dadada;");
            ...
            layout->addWidget(noContactsYet_);
            {
                ...
                l->setContentsMargins(Utils::scale_value(0), Utils::scale_value(0), Utils::scale_value(1), Utils::scale_value(0));
                l->setSpacing(Utils::scale_value(0));
                {
                    auto uw = new QWidget(noContactsYet_);
                    auto uwl = new QVBoxLayout(uw);
                    uw->setStyleSheet("background-color: rgba(255, 255, 255, 90%);");
    ...
    void ContactList::showNoRecentsYet(QWidget *parent, QWidget *list, QLayout *layout, std::function<void()> action)
    {
        ...
        if (!noRecentsYet_)
        {
            ...
            noRecentsYet_->setStyleSheet("background-color: #dadada;");
                ...
                l->setSpacing(Utils::scale_value(0));
                {
                    ...
                    }
                    {
                        ...
                        pl->setAlignment(Qt::AlignCenter);
                        {
                            ...
                            {
                                QString s = "QPushButton { font-family: \"%FONT_FAMILY%\"; font-weight: %FONT_WEIGHT%; color: #579e1c; font-size: 18dip; background-color: #66cae6b3; border: none; padding-left: 24dip; padding-right: 24dip; min-height: 40dip, max-height: 40dip; } QPushButton:hover { color: #ffffff; background-color: #579e1c; }";
                                Utils::SetFont(&s);
                                w->setStyleSheet(Utils::ScaleStyle(s, Utils::get_scale_coefficient()));
                            }
    ...
    {
        showNoRecentsYet(recents_page_, recents_view_, vertical_layout_3_, [this](){ changeTab(CurrentTab::ALL); });
    }
