...
namespace Ui
{
    ...
    void MessageItem::drawDeliveryStatusAndTime(QPainter &p)
    {
        p.save();
        ...
        if (ContentWidget_)
        {
            ...
            const auto fullStatusLineWidth = (
                evaluateStatusLineWidth() +
                getTimeStatusMargin()
            );
        ...
        {
            cursorX = posHint.x();
            cursorX += evaluateLeftContentMargin();
            cursorX += getTimeStatusMargin();
        }
        else
        {
            cursorX =
                evaluateContentHorGeometry(
                evaluateContentWidth(width())
            ).right();

            cursorX -= evaluateStatusLineWidth();
        }

        auto statusBaseline = 0;
        if (posHint.y() > 0)
        {
            statusBaseline = posHint.y();
            statusBaseline += evaluateTopContentMargin();
        }
        else
        {
            statusBaseline = evaluateContentVertGeometry().bottom();
        }

        statusBaseline -= getTimeStatusMargin();

        if (isOutgoing())
        {
            const auto resId = getDeliveryStatusResId();

            const auto statusPixmap = Themes::GetPixmap(resId);
            assert(statusPixmap);

            auto statusY = statusBaseline;
            statusY -= statusPixmap->GetHeight();
            statusY += getStatusIconBottomPadding(resId);
        ...
        p.drawText(cursorX, statusBaseline, TimeText_);
    ...
    void MessageItem::drawMessageBubble(QPainter &p)
    {
        ...
        p.fillPath(Bubble_, getBodyBrush(Data_->Outgoing_, isSelected()));
    }
    ...
    int32_t MessageItem::evaluateStatusLineWidth() const
    {
        auto width = getTimeStatusMargin(); // right margin

        assert(TimeTextSize_.isValid());
        width += TimeTextSize_.width();
    ...
    void MessageItem::updateMessageBodyHorGeometry(const QRect &bubbleHorGeometry)
    {
        ...
        messageBodyWidth -= evaluateStatusLineWidth();
	...
	void MessageItem::setTime(const int32_t time)
	{
        ...
        TimeText_ = QDateTime::fromTime_t(time)
                        .time()
                        .toString("HH:mm");

        QFontMetrics m(getTimeFont());

        TimeTextSize_ = m.tightBoundingRect(TimeText_).size();
        assert(TimeTextSize_.isValid());
	}
