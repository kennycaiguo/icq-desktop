...
namespace Ui
{
        ...
        , Shadow_(0)
	{
        ...
        {
            stacked_widget_->setImage(":/resources/main_window/pat_100.png");
        }
        else if (platform::is_windows())
        {
            QPalette pal = stacked_widget_->palette();
            pal.setBrush(QPalette::Background, QPixmap(":/resources/main_window/pat_100.png"));
            stacked_widget_->setPalette(pal);
            stacked_widget_->setAutoFillBackground(true);
        }
        else
        {
            stacked_widget_->setProperty("MainWidget", QVariant(true));
        }
	...
	{        
        if (isMaximized())
        {
            ...
            get_gui_settings()->set_value(settings_main_window_rect_real, QRect(pos().x(), pos().y(), event->size().width(), event->size().height()));
        }
        else
        {
            ...
            get_gui_settings()->set_value(settings_main_window_rect_real, QRect(rc.topLeft().x(), rc.topLeft().y(), event->size().width(), event->size().height()));
        }
	...
	void MainWindow::moveEvent(QMoveEvent* event)
	{
        ...
        auto rc = Ui::get_gui_settings()->get_value(settings_main_window_rect_real, QRect());
        rc.moveTo(event->pos().x(), event->pos().y());
        Ui::get_gui_settings()->set_value(settings_main_window_rect_real, rc);
	}
	...
	void MainWindow::initSettings()
	{
		...
		if (main_rect.left() == 0 && main_rect.top() == 0)
		{
            ...
            get_gui_settings()->set_value(settings_main_window_rect_real, geometry());
		}
        else
        {
        ...
        if (platform::is_linux())
		    Utils::addShadowToWindow(this, !get_gui_settings()->get_value<bool>(settings_window_maximized, false));
	}
	...
	void MainWindow::maximize()
	{
        ...
        if (platform::is_linux())
		    Utils::addShadowToWindow(this, !isMaximized());
    ...
    void MainWindow::minimize()
    {
        ...
        GetDispatcher()->post_stats_to_core(core::stats::stats_event_names::main_window_minimize);
    }
    ...
    void MainWindow::quote()
    {
        ...
        const QString & aimId = main_page_->getContactDialog()->currentAimId();
    ...
    {
        activate();
        main_page_->settingsTabActivate(SETTINGS_ABOUT);
    }
    
    void MainWindow::activateProfile()
    {
        activate();
        main_page_->settingsTabActivate(SETTINGS_PROFILE);
    }
    ...
    void MainWindow::hideWindow()
    {
        ...
        GetDispatcher()->post_stats_to_core(core::stats::stats_event_names::main_window_close);
    }
}
