    ...
    const wim_packet_params& _params)
{
    ...
    {
        core::http_request_simple request(core::proxy_settings(), _params.stop_handler_);
...
std::shared_ptr<download_file_sharing_preview_handler> loader::download_file_sharing_preview(
    ...
    const wim_packet_params& _params)
{
    std::wstring file_name = _previews_folder + L"/" + core::tools::from_utf8(core::tools::md5(_file_url.c_str(), (uint32_t)_file_url.length()));
    ...
    })->on_result_ = [handler, preview_data](int32_t _error)
    {
        ...
        {
            handler->on_result(_error, preview_data);
        }
    };
