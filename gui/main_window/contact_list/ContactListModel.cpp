...
namespace Logic
{
	...
	void ContactListModel::setCurrent(QString aimdId, bool sel)
	{
		CurrentAimdId_ = aimdId;
		if (sel)
			emit select(aimdId);
		else
	...
	void ContactListModel::get_contact_profile(const QString& _aimId, std::function<void(profile_ptr)> _call_back)
	{
		profile_ptr profile;
    ...
    void ContactListModel::block_spam_contact(const QString& _aimid)
    {
        Ui::gui_coll_helper collection(Ui::GetDispatcher()->create_collection(), true);
        collection.set_value_as_qstring("contact", _aimid);
