...
namespace
{
    ...
    bool containtPttAudio(const QString& text, Out int& duration);
...
namespace Data
{
    ...
    bool MessageBuddy::ContainsPttAudio() const
    {
        ...
        int duration;
        return containtPttAudio(Text_, duration);
    }
	...
	{
		return (IsFileSharing() || IsSticker() || IsChatEvent() || IsVoipEvent());
	}
    ...
    {
        int duration;
        containtPttAudio(GetText(), duration);
        return duration;
    }
...
namespace
{
		...
		const qint64 theirs_last_read)
	{
		...
		message->SetTime(msgColl.get_value_as_int("time"));
        ...
        if (msgColl.is_value_exist("voip"))
        {
            ...
            message->SetVoipEvent(
                HistoryControl::VoipEventInfo::Make(voip)
            );
        }
