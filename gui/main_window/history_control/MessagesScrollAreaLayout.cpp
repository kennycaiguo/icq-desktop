...
namespace Ui
{
    ...
    MessagesScrollAreaLayout::ItemInfo::ItemInfo(QWidget *widget)
        : Widget_(widget)
    {
        assert(Widget_);
    }
    ...
    void MessagesScrollAreaLayout::insertWidget(const int32_t pos, QWidget *widget)
    {
        assert(widget);
        ...
        auto itemInfoIter = insertItem(widget, pos);
        ...
        // -----------------------------------------------------------------------
        // recalculate items rectangles and apply bottom scroll lock (if needed)

        if (isAtBottom && (slideOp != SlideOp::SlideUp))
        {
            moveViewportToBottom();
        }
    ...
    MessagesScrollAreaLayout::ItemsInfoIter MessagesScrollAreaLayout::insertItem(QWidget *widget, const int32_t position)
    {
        assert(widget);
        assert(position >= 0);
        assert(position <= (int32_t)LayoutItems_.size());
        ...
        if (position == 0)
        {
            __TRACE(
                "geometry",
                "    pos=<" << position << ">\n"
                "    item inserted to front (geometry bottom)"
            );

            LayoutItems_.emplace_front(widget);
            return LayoutItems_.begin();
        }

        if (position == (int32_t)LayoutItems_.size())
        {
            __TRACE(
                "geometry",
                "    pos=<" << position << ">\n"
                "    item inserted to back (geometry top)"
            );

            LayoutItems_.emplace_back(widget);
            return (LayoutItems_.end() - 1);
        }

        __TRACE(
            "geometry",
            "    pos=<" << position << ">\n"
            "    item inserted in the middle (geometry below pos)"
        );

        auto iter = LayoutItems_.begin();
        iter += position;

        return LayoutItems_.emplace(iter, widget);
    }
