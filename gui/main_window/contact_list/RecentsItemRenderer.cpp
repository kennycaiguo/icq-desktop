...
namespace
{
	...
	namespace L
	{
		...
		const auto messageHeight = dip(22);
		const auto messageX = nameX;
		const auto messageY = dip(31);
...
namespace
{
	...
	void RenderContactMessage(QPainter &painter, const RecentItemVisualData &visData, const int rightBorderPx)
	{
...
#endif
		QFontMetrics m(font);
        QString text = visData.Status_.trimmed().replace("\n", " ");
        
        const auto senderNick = visData.senderNick_;
        int boldN = 0;
        if (!senderNick.isEmpty() && !visData.IsTyping_)
        {
            text = QString("%1: %2").arg(senderNick, text);
            boldN = senderNick.length() + 1;
        }
		const auto elidedString = m.elidedText(text, Qt::ElideRight, maxWidth);
        ...
        if (boldN)
        {
            ...
            cursor.movePosition(QTextCursor::Right, QTextCursor::KeepAnchor, boldN);
            QString s = cursor.selectedText();
            cursor.insertHtml("<font style=\"font-family: " +
                              Utils::appFontFamily(Utils::FontsFamily::SEGOE_UI_SEMIBOLD) +
                              "; font-weight: " +
                              Utils::appFontWeightQss(Utils::FontsFamily::SEGOE_UI_SEMIBOLD) +
                              ";\">" + s + "</font>");
        }
        ...
        if (platform::is_apple())
        {
            ...
            qreal expHeight = L::messageHeight.px();
            qreal correction = (expHeight > realHeight)?3:expHeight - realHeight;
	...
	void RenderContactName(QPainter &painter, const RecentItemVisualData &visData, const int rightBorderPx)
	{
        ...
        {
            qreal realHeight = doc.documentLayout()->documentSize().toSize().height();
            qreal expHeight = L::nameHeight.px();
            qreal correction = (expHeight > realHeight)?0:expHeight - realHeight + 1;
