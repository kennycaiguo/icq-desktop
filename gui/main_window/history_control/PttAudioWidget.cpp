...
namespace HistoryControl
{
    ...
    void PttAudioWidget::initialize()
    {
        ...
        ProgressPath_ = getBodyPath(QRect(0, 0, Utils::scale_value(widget_width), Utils::scale_value(widget_height)), Utils::scale_value(border_radious), IsOutgoing_);
        QTime time;
        time.setHMS(0, 0, Duration_ / 1000);
        ...
        MetaDownloadId_ = Ui::GetDispatcher()->downloadSharedFile(Contact_, Uri_, Ui::get_gui_settings()->get_value<QString>(settings_download_directory, ""), core::file_sharing_function::download_meta);
    }
    ...
    void PttAudioWidget::render(QPainter &p)
    {
        ...
        if (HaveText_)
        {
            additionalHeight += Utils::scale_value(text_bottom_margin);
            additionalHeight += Utils::scale_value(text_top_margin);
        }
        p.fillPath(getBodyPath(QRect(0, 0, Utils::scale_value(widget_width), height() + additionalHeight), Utils::scale_value(border_radious), IsOutgoing_), getBodyBrush(IsOutgoing_, Selected_));
        p.fillPath(ProgressPath_, getProgressBrush(IsOutgoing_));
        ...
        if (TextDownloading_)
        {
            ...
            const auto baseAngle = (AngleValue_ * 16);
            p.drawArc(QRect(Utils::scale_value(widget_width) - Utils::scale_value(text_button_right_margin) - Utils::scale_value(text_button_size) + penWidth / 2, Utils::scale_value(text_button_top_margin) + penWidth / 2, 
        ...
        else
        {
            ...
            Utils::check_pixel_ratio(pixmap);
            p.drawPixmap(Utils::scale_value(widget_width) - Utils::scale_value(text_button_right_margin) - pixmap.width() / ratio, Utils::scale_value(text_button_top_margin), pixmap);
        }
        ...
        if (HaveText_)
        {
            QPen pen(QBrush(QColor(0x0, 0x0, 0x0, (int32_t)(0.15 * 255))), Utils::scale_value(1));
            p.setPen(pen);
            p.drawLine(QPoint(0, Utils::scale_value(widget_height)), QPoint(Utils::scale_value(widget_width), Utils::scale_value(widget_height)));
    ...
    {
        return QPoint(Utils::scale_value(widget_width), 0);
    }
        ...
        if (isMouseOnPlayButton(e->pos()))
        {
            ...
            else
            {
                if (LocalPath_.isEmpty())
                {
                    ...
                    DownloadId_ = Ui::GetDispatcher()->downloadSharedFile(Contact_, Uri_, Ui::get_gui_settings()->get_value<QString>(settings_download_directory, ""), core::file_sharing_function::download_file);
    ...
    void PttAudioWidget::setFileInfo(qint64 id, QString, QString, QString, QString, QString, qint64 size, bool played)
    {
    ...
    void PttAudioWidget::speechToText(qint64 seq, int error, QString text)
    {
        if (seq == TextSeq_)
        {
            TextDownloading_ = false;
            ...
            if (!PttText_)
            {
                PttText_ = new QLabel(this);
                PttText_->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Preferred);
                PttText_->setFixedWidth(Utils::scale_value(widget_width) - Utils::scale_value(play_button_left_margin) - Utils::scale_value(text_button_right_margin));
                PttText_->setStyleSheet("background: transparent;");
                PttText_->setFrameStyle(QFrame::NoFrame);
                QFont f = Utils::appFont(Utils::FontsFamily::SEGOE_UI, Utils::scale_value(15));
                PttText_->setFont(f);
                PttText_->setWordWrap(true);
                PttText_->setTextFormat(Qt::PlainText);
                QPalette pal;
                pal.setColor(QPalette::Text, QColor("#282828"));
                PttText_->setPalette(pal);
            }
            disconnect(Ui::GetDispatcher(), SIGNAL(speechToText(qint64, int, QString)), this, SLOT(speechToText(qint64, int, QString)));
            if (error != 0 || text.isEmpty())
                PttText_->setText(QT_TRANSLATE_NOOP("ptt_widget", "unclear message"));
            else
    ...
    {
        return pos.x() >= Utils::scale_value(widget_width) - Utils::scale_value(text_button_right_margin) - Utils::scale_value(text_button_size)
               && pos.x() <= Utils::scale_value(widget_width) - Utils::scale_value(text_button_right_margin)
    ...
    void PttAudioWidget::showText()
    {
        if (!PttText_)
        {
            ...
            connect(Ui::GetDispatcher(), SIGNAL(speechToText(qint64, int, QString)), this, SLOT(speechToText(qint64, int, QString)), Qt::QueuedConnection);
    ...
    QBrush PttAudioWidget::getBodyBrush(bool isOutgoing, bool isSelected)
    {
    ...
    QBrush PttAudioWidget::getProgressBrush(bool isOutgoing)
    {
        if (ProgressValue_ == 0 || ProgressValue_ == Duration_)
            return QBrush(QColor(0x0, 0x0, 0x0, 0));
        ...
        grad.setColorAt(0.00, color0);
        double pos = ((double)ProgressValue_ / (double)Duration_);
        grad.setColorAt(pos, color0);
        grad.setColorAt(pos + 0.01, QColor(0x0, 0x0, 0x0, 0));
        grad.setColorAt(1.00, QColor(0x0, 0x0, 0x0, 0));
