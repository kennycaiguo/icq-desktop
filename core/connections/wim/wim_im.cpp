...
void im::download_stickers_metadata(int64_t _seq, const std::string _size)
{
    ...
    get_stickers()->get_next_meta_task()->on_result_ = [wr_this, _size, _seq](bool _res, const stickers::download_task& _task)
    {
        ...
        if (!_res)
        {
            ...
            ptr_this->get_stickers()->serialize_meta(_coll, _size)->on_result_ = [wr_this, _seq](coll_helper _coll)
            {
                auto ptr_this = wr_this.lock();
                if (!ptr_this)
                    return;
...
void im::get_stickers_meta(int64_t _seq, const std::string& _size)
{
    ...
    get_stickers()->load_from_local()->on_result_ = [wr_this, stickers_size, _seq](const stickers::load_result& _res)
    {
        auto ptr_this = wr_this.lock();
        if (!ptr_this)
            return;
...
void im::get_sticker(int64_t _seq, int32_t _set_id, int32_t _sticker_id, core::sticker_size _size)
{
    ...
    get_stickers()->get_sticker(_seq, _set_id, _sticker_id, _size)->on_result_ =
        [_seq, _set_id, _sticker_id, _size](tools::binary_stream& _sticker_data)
    {
...
void core::wim::im::poll(bool _is_first, bool _after_network_error, int32_t _failed_network_error_count)
{
    ...
    fetch_thread_->run_async_task(packet)->on_result_ = [_is_first, _after_network_error, active_session_id, packet, wr_this, _failed_network_error_count](int32_t _error)
    {
        ...
        if (_error == 0)
        {
            for (auto &event : packet->get_events())
            {
                event->on_im(ptr_this);
            }
...
void core::wim::im::on_event_buddies_list(fetch_event_buddy_list* _event)
{
	...
	if (contact_list)
	{
        ...
        get_archive()->sync_with_history()->on_result_ = [contact_list, wr_this](int32_t _error)
        {
            auto ptr_this = wr_this.lock();
            if (!ptr_this)
                return;
...
void core::wim::im::on_event_diff(fetch_event_diff* _event)
{
    auto diff = _event->get_diff();

    std::weak_ptr<im> wr_this(shared_from_this());

    get_archive()->sync_with_history()->on_result_ = [diff, wr_this](int32_t _error)
    {
        auto ptr_this = wr_this.lock();
        if (!ptr_this)
            return;
...
void core::wim::im::on_event_my_info(fetch_event_my_info* _event)
{
	my_info_cache_->set_info(_event->get_info());
	post_my_info_to_gui();
}

void core::wim::im::on_event_user_added_to_buddy_list(fetch_event_user_added_to_buddy_list* _event)
{
}

void core::wim::im::on_event_typing(fetch_event_typing *_event)
{
...
void core::wim::im::on_event_presence(fetch_event_presence* _event)
{
...
std::shared_ptr<async_task_handlers> im::post_dlg_state_to_gui(const std::string _contact)
{
	auto handler = std::make_shared<async_task_handlers>();

	std::weak_ptr<im> wr_this = shared_from_this();

	get_archive()->get_dlg_state(_contact)->on_result = [wr_this, handler, _contact](const archive::dlg_state& _state)
	{
		auto ptr_this = wr_this.lock();
		if (!ptr_this)
			return;
...
void im::on_event_dlg_state(fetch_event_dlg_state* _event)
{
	...
	get_archive()->set_dlg_state(aimid, server_dlg_state)->on_result = [wr_this, aimid, server_dlg_state]()
	{
		auto ptr_this = wr_this.lock();
		if (!ptr_this)
		{
		...
		ptr_this->post_dlg_state_to_gui(aimid);
	};
	...
	if (has_opened_dialogs(aimid))
	{
		...
		serialize_messages_4_gui("messages", messages, coll.get(), auth_params_->time_offset_);
	...
	remove_messages_from_not_sent(get_archive(), aimid, messages)->on_result_ = [wr_this, aimid, messages](int32_t _error)
	{
		auto ptr_this = wr_this.lock();
		if (!ptr_this)
			return;
...
void im::on_event_hidden_chat(fetch_event_hidden_chat* _event)
{
...
void core::wim::im::sign_url(int64_t _seq, const std::string& unsigned_url)
{
    std::string host = "https://www.icq.com/karma_api/karma_client2web_login.php";

    std::map<std::string, std::string> params;
    auto wim_params = make_wim_params();

    params["a"] = /*wim_packet::escape_symbols*/(wim_params.a_token_);
    params["k"] = /*wim_packet::escape_symbols*/(wim_params.dev_id_);
    params["ts"] = /*wim_packet::escape_symbols*/(tools::from_int64(std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()) - wim_params.time_offset_));
    params["d"] = wim_packet::escape_symbols(unsigned_url);
    params["owner"] = /*wim_packet::escape_symbols*/(wim_params.aimid_);
    params["sig_sha256"] = wim_packet::escape_symbols(wim_packet::get_url_sign(host, params, wim_params, false, false));

    std::stringstream ss_url;
    ss_url << host << wim_packet::params_map_2_string(params);
    std::string signed_url = ss_url.str();
...
void im::add_chat(int64_t _seq, const std::string& _m_chat_name, const std::string& _m_chat_members)
{
	std::weak_ptr<wim::im> wr_this(shared_from_this());
    ...
    post_wim_packet(packet)->on_result_ = [packet, __this](int32_t _error)
    {
        ...
        if (!!ptr_this) {
            ...
            if (!!response && response->available()) {
                ...
                ptr_this->on_voip_proto_msg(true, (const char*)response->read(size), size);
            }
        } else {
            assert(false);
        }
	...
	const file_sharing_function _function)
{
	...
	auto files_folder = get_im_downloads_path(_download_dir);
#ifdef _WIN32
...
#endif
	auto handler = get_loader().download_file_sharing(_seq, _file_url, _function, files_folder, previews_folder, make_wim_params());
...
void im::download_preview(int64_t _seq, const std::string& _file_url, const bool _sign_url)
{
	std::weak_ptr<im> wr_this = shared_from_this();

	std::wstring previews_folder = get_im_data_path() + L"/content.cache";

	auto handler = get_loader().download_file_sharing_preview(_seq, _file_url, previews_folder, _sign_url, make_wim_params())->on_result =
		[_seq, _file_url, wr_this](int32_t _error, std::shared_ptr<core::tools::binary_stream> _data)
		{
			coll_helper cl_coll(g_core->create_collection(), true);
...
void core::wim::im::on_event_permit(fetch_event_permit *_event)
{
