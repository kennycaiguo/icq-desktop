...
struct curl_context
{
    ...
    const proxy_settings& proxy_settings_;
    ...
    curl_context(std::function<bool()> _stop_func, bool _keep_alive, const proxy_settings& _proxy_settings)
        :	
    ...
    bool init(int32_t _connect_timeout, int32_t _timeout)
    {
        if (!curl_)
            return false;
        ...
        {
            curl_easy_setopt(curl_, CURLOPT_PROXY, tools::from_utf16(proxy_settings_.proxy_server_).c_str());
            curl_easy_setopt(curl_, CURLOPT_PROXYTYPE, proxy_settings_.proxy_type_);

            if (proxy_settings_.need_auth_)
            {
                curl_easy_setopt(curl_, CURLOPT_PROXYAUTH, CURLAUTH_ANY);
                curl_easy_setopt(curl_, CURLOPT_PROXYUSERPWD, tools::from_utf16(proxy_settings_.login_ + L":" + proxy_settings_.password_).c_str());
            }
        }
...
static void dump_function(const char *text, std::stringstream& _ss_out, unsigned char *ptr, size_t size, bool nohex)
{
    ...
    for(i=0; i<size; i+= width) {
        ...
        for(c = 0; (c < width) && (i+c < size); c++) {
            ...
            sprintf(buffer, "%c", (ptr[i+c]>=0x20) && (ptr[i+c]<0x80)?ptr[i+c]:'.');
            _ss_out << buffer;
...
static int32_t trace_function(CURL* _handle, curl_infotype _type, unsigned char* _data, size_t _size, void* _userp)
{
    ...
    case CURLINFO_TEXT:
        {
        //    static std::vector<std::string> filters = get_filter_keywords();
        //    for (const std::string& _filter : filters)
        //    {
        //        if (_size > _filter.length() && memcmp(_filter.c_str(), _data, _filter.length()) == 0)
        //            return 0;
        //    }
...
//        __nop();
    ...
    dump_function(text, ss, _data, _size, true);

    __NET("thread=0x%1%\n%2%", boost::this_thread::get_id() % ss.str());

    return 0;	
}


http_request_simple::http_request_simple(const proxy_settings& proxy_settings, std::function<bool()> stop_func)
    :	is_stop_(stop_func),
...
{
    curl_context ctx(is_stop_, keep_alive_, proxy_settings_);

    if (!ctx.init(connect_timeout_, timeout_))
    {
        assert(false);
        return false;
    }
