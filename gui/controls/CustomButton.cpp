...
Ui::CustomButton::CustomButton(QWidget* _parent, const QString& _imageName)
    : QPushButton(_parent), x_(0), y_(0), xForActive_(NO_VALUE), yForActive_(NO_VALUE), align_(Qt::AlignCenter), fillColor_(QColor())
{
    pixmapDefault_ = QPixmap(Utils::parse_image_name(_imageName));
...
{
    pixmapHover_ = QPixmap(Utils::parse_image_name(_imageName));
}

void Ui::CustomButton::setActiveImage(const QString& _imageName)
{
    pixmapActive_ = QPixmap(Utils::parse_image_name(_imageName));
}
